...
if FLAGS.model in ("fully_connected_32x32x3", "fully_connected_84x84x3", "fully_connected_84x84x4"):
  CURRICULUM_STEPS = [150000, 80000]
  CURRICULUM_SEQ = [1, 20]
  CURRICULUM_BATCH_SIZE = [64, 16]
  CURRICULUM_LEARNING_RATE = [2e-5, 5e-6]
...
def train(iteration):
  ...
  with tf.Graph().as_default():
    ...
    if iteration != 0 or FLAGS.load_network:
      ...
      ckpt = tf.train.get_checkpoint_state(FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain")
    ...
    summary_writer = tf.train.SummaryWriter(FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain", graph_def=graph_def)
    ...
    for step in xrange(CURRICULUM_STEPS[iteration]):
      ...
      if step%1000 == 0:
        checkpoint_path = os.path.join(FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain", 'model.ckpt')
        saver.save(sess, checkpoint_path, global_step=step)  
        print("saved to " + FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain")

def main(argv=None):  # pylint: disable=unused-argument
  if not FLAGS.load_network:
    if tf.gfile.Exists(FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain"):
      tf.gfile.DeleteRecursively(FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain")
    tf.gfile.MakeDirs(FLAGS.train_dir + FLAGS.model + FLAGS.system + "_pretrain")
