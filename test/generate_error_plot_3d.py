...
from utils.numpy_divergence import divergence_3d
from utils.numpy_drag import drag_3d
from utils.numpy_flux import flux_3d
  ...
  with tf.Graph().as_default():
    ...
    for sim in tqdm(xrange(FLAGS.test_nr_runs)):
      for run in xrange(FLAGS.test_nr_per_simulation):
        # get frame
        state_feed_dict, boundary_feed_dict = generate_feed_dict(1, shape, FLAGS.lattice_size, 'fluid_flow_' + str(shape[0]) + 'x' + str(shape[1]) + 'x' + str(shape[2]) + '_test', sim, run+0)
        ...
        for step in tqdm(xrange(FLAGS.test_length)):
          ...
          # get true value
          state_feed_dict, boundary_feed_dict = generate_feed_dict(1, shape, FLAGS.lattice_size, 'fluid_flow_' + str(shape[0]) + 'x' + str(shape[1]) + 'x' + str(shape[2]) + '_test', sim, run+step+0)
          true_state = state_feed_dict[0]
          # calc error
          mse_error[sim, step] = np.sqrt(np.sum(np.square(generated_state - true_state)))
          # calc divergence
          divergence_true[sim, step] = np.sum(np.abs(divergence_3d(true_state[:,:,:-1])))
          divergence_generated[sim, step] = np.sum(np.abs(divergence_3d(generated_state[:,:,:-1])))
          # calc drag
          drag_generated_x[sim, step], drag_generated_y[sim, step], drag_generated_z[sim, step] = drag_3d(generated_state[:,:,:,0:3], generated_state[:,:,:,3], boundary_feed_dict[0,:,:,:,0])
          drag_true_x[sim, step], drag_true_y[sim, step], drag_true_z[sim, step] = drag_3d(true_state[:,:,:,0:3], true_state[:,:,:,3], boundary_feed_dict[0,:,:,:,0])
          # calc flux
          flux_generated_x[sim, step], flux_generated_y[sim, step], flux_generated_z[sim, step] = flux_3d(generated_state[:,:,:,0:3], generated_state[:,:,:,3], boundary_feed_dict[0,:,:,:,0])
          flux_true_x[sim, step], flux_true_y[sim, step], flux_true_z[sim, step] = flux_3d(true_state[:,:,:,0:3], true_state[:,:,:,3], boundary_feed_dict[0,:,:,:,0])
    ...
    drag_true_z_mean = np.sum(drag_true_z, axis=0)/FLAGS.test_nr_runs

    drag_true_z_std = np.sqrt(np.sum(np.square(drag_true_z - np.expand_dims(drag_true_z_mean, axis=0)), axis=0)/FLAGS.test_nr_runs)
    ...
    drag_generated_z_mean = np.sum(drag_generated_z, axis=0)/FLAGS.test_nr_runs

    drag_generated_z_std = np.sqrt(np.sum(np.square(drag_generated_z - np.expand_dims(drag_generated_z_mean, axis=0)), axis=0)/FLAGS.test_nr_runs)
    # flux
    ...
    flux_true_z_mean = np.sum(flux_true_z, axis=0)/FLAGS.test_nr_runs

    flux_true_z_std = np.sqrt(np.sum(np.square(flux_true_z - np.expand_dims(flux_true_z_mean, axis=0)), axis=0)/FLAGS.test_nr_runs)
    ...
    flux_generated_z_mean = np.sum(flux_generated_z, axis=0)/FLAGS.test_nr_runs

    flux_generated_z_std = np.sqrt(np.sum(np.square(flux_generated_z - np.expand_dims(flux_generated_z_mean, axis=0)), axis=0)/FLAGS.test_nr_runs)
    ...
    plt.legend()
    plt.figure(5)
    plt.errorbar(x, drag_true_z_mean, yerr=drag_true_z_std, label='z drag true')
    plt.errorbar(x, drag_generated_z_mean, yerr=drag_generated_z_std, label='z drag generated')
    plt.title('drag z')
    plt.legend()
    ...
    plt.legend()
    plt.figure(8)
    plt.errorbar(x, flux_true_z_mean, yerr=flux_true_z_std, label='z flux true')
    plt.errorbar(x, flux_generated_z_mean, yerr=flux_generated_z_std, label='z flux generated')
    plt.title('flux z')
    plt.legend()
