...
NUM_NETWORKS = 4
NETWORKS = ["../checkpoints/fluid_32_lstm_32x32x10fluid_compression_2kl_compression_vae_loss_False_sample_compression_False_lstm_size_100", "../checkpoints/fluid_32_lstm_32x32x10fluid_compression_2kl_compression_vae_loss_False_sample_compression_False_lstm_size_256", "../checkpoints/fluid_32_lstm_32x32x10fluid_all_kl_compression_vae_loss_False_sample_compression_False_lstm_size_100", "../checkpoints/fluid_32_lstm_32x32x10fluid_all_kl_compression_vae_loss_False_sample_compression_False_lstm_size_256"]
MODELS = ["lstm_32x32x10", "lstm_32x32x10", "lstm_32x32x10", "lstm_32x32x10"] 
LSTM_SIZE = [100, 256, 100, 256]
TRAIN_TYPE = ["compression", "compression", "final", "final"]
...
def evaluate(i):
  ...
  with tf.Graph().as_default():
    ...
    plt.figure(1)
    plt.subplot(311)
    plt.plot(error[4:], label="lstm size " + str(LSTM_SIZE[i]) + ' ' + TRAIN_TYPE[i])
    #if i == 0:
    #  plt.plot(error_blank, label="blank image")
    plt.title("mean squared error")
    plt.legend()
    plt.subplot(312)
    plt.plot(stddev[4:], label="lstm size " + str(LSTM_SIZE[i]) + ' ' + TRAIN_TYPE[i])
    plt.title("standard deviation")
    plt.legend()
    plt.subplot(313)
    plt.plot(error_mean[4:], label="lstm size " + str(LSTM_SIZE[i]) + ' ' + TRAIN_TYPE[i])
    plt.title("mean squared error lstm")
    plt.legend()

    plt.savefig('../../Compressing-Dynamical-Systems-Paper/figures/fluid_flow/fluid_flow_error.png')
