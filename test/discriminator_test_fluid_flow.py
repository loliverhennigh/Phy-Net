...
def train():
  ...
  with tf.Graph().as_default():
    ...
    for i in xrange(int(unroll_length/2)-1):
      # unroll generator network 
      x_2, hidden_state = ring_net.encode_compress_decode_gan(flow_boundry[:,i+int(unroll_length/2),:,:,:], hidden_state, z[:,i+int(unroll_length/2),:], keep_prob_encoding, keep_prob_lstm)
      x_2 = x_2 * boundry_kill[:,0,:,:,:]
      x_2_o.append(x_2)
    ...
    error_reconstruction = tf.nn.l2_loss(flow[:,int(unroll_length/2):,:,:,:] - x_2_o)
    ...
    for step in xrange(400000):
      ...
      if step%1000 == 0:
        checkpoint_path = os.path.join(FLAGS.checkpoint_dir + "_discriminator", 'model.ckpt')
