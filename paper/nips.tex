...
Comparing the accuracy of simulations is somewhat difficult. The simplest approach is to compare the MSE between the generated and true simulation at various timesteps. The problem with this approach is that fluid flow is a chaotic dynamical system and small perdibations in flow quickley compound leading to dramatic diffrences. For this reason, we develop a multi metric approach for evaluating the generated flows accuracy. Similar to \cite{} we compare the divergence of the generated and tru velocity vector field. These values should be small (need to come back to this). We also compare computed values from the flow such as drag and flux. Flow simulations are often run to calculate such values so comparing is a strong indecator of our models real world applicability. The drag is calculated with the momentum transfer method \cite{guo2013lattice}.

In figure \ref{2d_error_plot} we see the predicted values . We notice in the 2D case, the 

\subsection{Transferability to different datasets}

\begin{figure}[!t]
\centering
\subfigure{\includegraphics[scale=0.9]{../test/figs/256x512_2d_flow_image.png}}
\subfigure{\includegraphics[scale=0.4]{../test/figs/256x512_2d_error_plot.png}}
...
Finally, we illustrate the generatlity of our method by applying it to electromagnetic simulations. The same nueral network architecture is used as in the 2D flow simulations. The only diffrence in training is the 
