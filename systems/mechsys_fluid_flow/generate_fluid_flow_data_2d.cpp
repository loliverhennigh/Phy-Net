...
int main(int argc, char **argv) try
{
    ...
    while (h<num_objects && trys<1000)
    {
        ...
        {
	    // set inner obstacle
            int length_x = (rand() % 60) + 10;
            int length_y = (rand() % 60) + 10;
	    ...
	    double obsY   = (rand() % (ny-(2*max_length))) + (1.0*double(max_length)) ;   // y position
            ...
            for (size_t i=0;i<nx;i++)
            {
                for (size_t j=0;j<ny;j++)
                {
                    if (obsX-2.0*length_x < i && obsX + 2.0*length_x > i && obsY-2.0*length_y < j && obsY + 2.0*length_y > j)
                    {
                        if (Dom.IsSolid[0][i][j][0])
                        {
                            place_object = 0;
            ...
            if (place_object == 1)
            {
                ...
                for (size_t i=0;i<nx;i++)
                {
                    for (size_t j=0;j<ny;j++)
                    {
                        if (obsX < i && obsX + length_x > i && obsY < j && obsY + length_y > j)
                        {
                            Dom.IsSolid[0][i][j][0] = true;
                        }
                    }
