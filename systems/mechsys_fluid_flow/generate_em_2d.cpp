...
int main(int argc, char **argv) try
{
   ...
   //double beta  = 0.0005;
    ...
    size_t num_objects = 6*(nx/256)*(nx/256);
    ...
    while (h<num_objects && trys<1000)
    {
	    ...
	    double obsY   = (rand() % ((nz-int(z0))-(3*max_radius))) + (1.5*max_radius+int(z0)) ;   // y position
            ...
            for (size_t i=0;i<nx;i++)
            {
                ...
                {
                    Dom.Lat.GetCell(iVec3_t(i,0,j))->Eps = max(Dom.Lat.GetCell(iVec3_t(i,0,j))->Eps, 1.0*tanh(5.0*(-((pow(cos(alpha)*(i-obsX) + sin(alpha)*(j-obsY),2.0))/(radius_x*radius_x)+(pow(sin(alpha)*(i-obsX) - cos(alpha)*(j-obsY),2.0))/(radius_y*radius_y)) + 1.0)) + 3.0);
                }
            }
    ...
    Dom.Solve(2000.0,32.0,NULL,NULL,argv[2],true,nproc);
