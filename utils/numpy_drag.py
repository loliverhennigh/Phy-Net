    ...
    for j in xrange(boundary.shape[1]-2):
      if boundary[i+1,j+1] < 1.0:
        if boundary[i+2,j+1] > 0.0:
          print("x " + str(velocity_field[i+1,j+1,1]))
          print("density " + str(density_field[i+1,j+1]))
          force_x += .5*density_field[i+1,j+1]*pow(velocity_field[i+1,j+1,1],2)
        ...
        if boundary[i+1,j+2] > 0.0:
          ...
          force_y += .5*density_field[i+1,j+1]*pow(velocity_field[i+1,j+1,0],2)
      ...
      for k in xrange(boundary.shape[2]-2):
        if boundary[i+1,j+1,k+1] < 1.0:
          if boundary[i+2,j+1,k+1] > 0.0:
            force_x += .5*density_field[i+1,j+1,k+1]*pow(velocity_field[i+1,j+1,k+1,2],2)
  
          if boundary[i,j+1,k+1] > 0.0:
            force_x += .5*density_field[i+1,j+1,k+1]*pow(velocity_field[i+1,j+1,k+1,2],2)
  
          if boundary[i+1,j+2,k+1] > 0.0:
            force_y += .5*density_field[i+1,j+1,k+1]*pow(velocity_field[i+1,j+1,k+1,1],2)
  
          if boundary[i+1,j,k+1] > 0.0:
            force_y += .5*density_field[i+1,j+1,k+1]*pow(velocity_field[i+1,j+1,k+1,1],2)

          if boundary[i+1,j+1,k] > 0.0:
            force_y += .5*density_field[i+1,j+1,k+1]*pow(velocity_field[i+1,j+1,k+1,0],2)
